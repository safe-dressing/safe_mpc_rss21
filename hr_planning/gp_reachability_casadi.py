...
Adapted from https://github.com/befelix/safe-exploration/blob/master/safe_exploration/gp_reachability_casadi.py
...
def onestep_reachability(pR, pH, qH, beta, ssm, l_mu, l_sigma, t_z_gp=None,
    ...
    n_pH = pH.shape[0]
    assert pR.shape == pH.shape == (n_pH, 1)
    ...
    # Equation (35) = 1st ellipsoid as (33) + 2nd ellipsoid bounding (34).
        ...
        mu_0, var_0, jac_mu = ssm(x_bar.T, u_bar.T)

        # a_mu = npH x npH = 2x2 = A_{\mu_n}
        # (the paragraph above equation (29)).
        # n_x_in = npH = 2
        n_x_in = t_z_gp.shape[0]
        # a_mu = npH x npH = 2x2 = A_{\mu_n}
        # (the paragraph above equation (29)).
        a_mu = jac_mu[:, :n_x_in]
        ...
        assert a_mu.shape == MX.eye(n_pH).shape
        H = a_mu + MX.eye(n_pH)
        qH_new_e1 = mtimes(H, mtimes(qH, H.T))
        ...
        if prop_vel:
            ...
            # npHxnpH
            H_for_vel = a_mu / dt
            vqH_new_e1 = mtimes(H_for_vel, mtimes(qH, H_for_vel.T))
        ...
        if prop_vel:
            ...
            assert v_lagrange_mu.shape == p_lagrange_mu.shape
            assert vQ_lagrange_mu.shape == Q_lagrange_mu.shape

            vQ_lagrange_sigm = ellipsoid_from_rectangle(b_sigma_eps_vel)
            v_lagrange_sigm = np.zeros((n_pH, 1))
            assert v_lagrange_sigm.shape == p_lagrange_sigm.shape
            assert vQ_lagrange_sigm.shape == Q_lagrange_sigm.shape

            v_sum_lagrange, vQ_sum_lagrange = sum_two_ellipsoids(
                    v_lagrange_sigm, vQ_lagrange_sigm,
                    v_lagrange_mu, vQ_lagrange_mu)
            assert v_sum_lagrange.shape == p_sum_lagrange.shape
            assert vQ_sum_lagrange.shape == Q_sum_lagrange.shape
        ...
        if prop_vel:
            ...
            assert v_1.shape == p_1.shape
            assert vq_1.shape == q_1.shape
...
def multi_step_reachability(pH_0, qH_0, pR_0, pRs_1_T, gp, l_mu, l_sigma,
    ...
    if prop_vel:
        assert pH_new.shape == vH_new.shape == pH_0.shape
        assert qH_new.shape == vqH_new.shape
    ...
    if prop_vel:
        ...
        assert vHs_1_T.shape == pHs_1_T.shape
        assert vqHs_1_T.shape == qHs_1_T.shape
    ...
    for t_minus_1 in range(h_safe - 1):
        ...
        pR_old = cas_reshape(pRs_1_T[t_minus_1, :], (n_pR, 1))
        pH_new, qH_new, pH_gp_pred_sigma_new, vH_new, vqH_new\
            = onestep_reachability(
                pR=pR_old, pH=pH_old, qH=qH_old, beta=beta,
                ssm=gp, l_mu=l_mu, l_sigma=l_sigma, t_z_gp=t_z_gp,
                prop_vel=prop_vel, dt=dt)
        pHs_1_T = vertcat(pHs_1_T, pH_new.T)
        qHs_1_T = vertcat(qHs_1_T, cas_reshape(qH_new, (1, n_pH * n_pH)))
        pH_gp_pred_sigma_1_T = vertcat(
                pH_gp_pred_sigma_1_T, pH_gp_pred_sigma_new)

        if prop_vel:
            assert pH_new.shape == vH_new.shape
            assert qH_new.shape == vqH_new.shape
            vHs_1_T = vertcat(vHs_1_T, vH_new.T)
            vqHs_1_T = vertcat(vqHs_1_T, cas_reshape(vqH_new, (1, n_pH * n_pH)))
...
def lin_ellipsoid_safety_distance(p_center, q_shape, h_mat, h_vec, c_safety=1.0):
    ...
    m, n_s = h_mat.shape
    assert p_center.shape == (n_s, 1), "p_center has to have shape n_s x 1"
    assert q_shape.shape == (n_s, n_s), "q_shape has to have shape n_s x n_s"
    assert h_vec.shape == (m, 1), "h_vec has to have shape m x 1"
