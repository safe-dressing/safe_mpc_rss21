...
class SimpleGPModel(StateSpaceModel):
    ...
    def from_dict(cls, gp_dict):
        ...
        if "data_path" in gp_dict and not gp_dict["data_path"] is None:
            ...
            print_FAIL("""In order to be trained, GP either needs a data_path or
    ...
    def train(self, X, y, m=None, opt_hyp=True, noise_diag=1e-5, Z=None,
        ...
        if not m is None:
            if self.do_sparse_gp and not Z is None:
                ...
                if n_data < m:
                    print_FAIL("""The desired number of datapoints is not available. Dataset consist of {}
    ...
    def update_model(self, x, y, opt_hyp, replace_old, noise_diag=1e-5,
                     choose_data=True):
    ...
    def sample_from_gp(self, inp, size=10):
        ...
        for i in range(self.n_s_out):
            tmp = self.gps[i].posterior_samples_f(inp, size=size, full_cov=False)
            tmp2 = np.squeeze(tmp, axis=1)
            assert tmp2.shape == S[:, :, i].shape
            S[:, :, i] = tmp2
